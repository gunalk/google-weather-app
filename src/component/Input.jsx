import React from 'react'
import "./Input.css"

const Input = ({input,setInput,findWeather}) => {
  const handleChange=(e)=>{
    setInput(e.target.value)
  }
  const handleClick=(event)=>{

   findWeather()
  }
  console.log(input)
  return (
    <div className='inputBox'>
        <div className='inputBox-field'>
            <h1>Google Weather App</h1>
            <p>Powered by</p>
            <img src="data:image/png;base64,"/>
         
            <input onChange={handleChange} value={input} style={{border:"none",outline:"none",borderBottom:
          "1px solid lightGray",width:"300px",marginLeft:"30px",fontSize:"22px",textAlign:"center"}} type="text" placeholder='Enter your city'/>
            <button onClick={handleClick} style={{width:"100px",marginLeft:"140px",marginTop:"30px"}} className="btn btn-success">search</button>
          
        </div>
    </div>
  )
}

export default Input